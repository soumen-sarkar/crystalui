...
class Pieces {
  constructor(el, options) {
    ...
    this.options = {
      ...
      animationDefaults: {
        ...
        translateX: (t, i) => {
          return t.dataset.column < this.getTotalColumns() / 2
            ? anime.random(50, 100) + "px"
            : anime.random(-100, -50) + "px";
        },
        translateY: (t, i) => {
  ...
  layout() {
    ...
    this.imgsrc = this.DOM.el.style.backgroundImage
      .replace("url(", "")
      .replace(")", "");
    // The background image.
    this.DOM.el.style.backgroundImage =
      this.options.bgimage !== "none" ? `url(${this.options.bgimage})` : "none";
  ...
  createPiece(row, column) {
    ...
    piece.style.backgroundSize = `${w * this.options.pieces.columns +
      4}px auto`;
    piece.dataset.row = row;
    piece.dataset.column = column;
    piece.dataset.delay =
      this.options.delay instanceof Array
        ? anime.random(this.options.delay[0], this.options.delay[1])
        : this.options.delay;
  ...
  initEvents() {
    ...
    const onMouseMoveFn = ev => {
      requestAnimationFrame(() => {
        ...
        //   };
        //   const mouseposScroll = {
        //     x: mousepos.x - docScrolls.left,
        //     y: mousepos.y - docScrolls.top
        //   };
        //   const rotX =
        //     ((2 * this.options.tilt.maxRotationX) / this.win.height) *
        //       mouseposScroll.y -
        //     this.options.tilt.maxRotationX;
        //   const rotY =
        //     ((2 * this.options.tilt.maxRotationY) / this.win.width) *
        //       mouseposScroll.x -
        //     this.options.tilt.maxRotationY;
        //   const transX =
        //     ((2 * this.options.tilt.maxTranslationX) / this.win.width) *
        //       mouseposScroll.x -
        //     this.options.tilt.maxTranslationX;
        //   const transY =
        //     ((2 * this.options.tilt.maxTranslationY) / this.win.height) *
        //       mouseposScroll.y -
    ...
    const onResizeFn = debounce(() => {
      ...
      for (let i = 0, len = this.pieces.length; i < len; i++) {
        const w = Math.round(
          this.dimensions.width / this.options.pieces.columns
        );
        const h = Math.round(this.dimensions.height / this.options.pieces.rows);
