...
function portfolioCanvas() {
  ...
  var textEl = [].slice.call(document.querySelectorAll(".pieces-subTitle__text"));
  var textE2 = [].slice.call(document.querySelectorAll(".pieces-title__text"));
  ...
  var captionOptions = {
    ...
    translate: function() {
      if (windowWidth > 1120) return { translateX: -350, translateY: -180 };
      if (windowWidth > 720) return { translateX: -300, translateY: -180 };
      return { translateX: -140, translateY: -75 };
    }
  };
  ...
  for (var i = 0; i < slidesLength; i++) {
    ...
    items.push({
      type: "text",
      value: textE2[i].innerText,
      options: captionOptions // numberOptions
    });

    // Save indexes
    textIndexes.push(i * 3 + 1);
    captionIndexes.push(i * 3 + 1);
